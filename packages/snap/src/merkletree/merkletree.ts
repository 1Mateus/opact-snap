...
const calculateMerkleDefault = async (poseidon) => {
  ...
  for (let i = 1; i < maxheight; i++) {
    (async () => {
      merkleDefaults[i] = poseidon.F.toObject(poseidon([merkleDefaults[i - 1], merkleDefaults[i - 1]]))
    })()
  }
...
export class MerkleTree {
  ...
  static async build(height: any) {
    ...
    const { merkleDefaults } = await calculateMerkleDefault(poseidon)

    return new MerkleTree(height, merkleDefaults, poseidon)
  }
    ...
    for (let i = 1; i < this.height; i++) {
      ...
      for (let j = from; j <= to; j++) {
        this._setcell(
          i,
          j,
          this.poseidon.F.toObject(this.poseidon([this._cell(i - 1, j * 2), this._cell(i - 1, j * 2 + 1)]))
        );
      }
    }
    this.length += s;
    ...
    if (elementssz == 0n) {
      ...
      for (let i = 1n; i < proofsz; i++) {
        ...
        for (let j = 0n; j < buffsz >> 1n; j++) {
          buff[offset + j] = this.poseidon.F.toObject(this.poseidon([buff[j * 2n], buff[j * 2n + 1n]]))
        }
  ...
  static getRoot(proof: any, index: any, leaf: any) {
    ...
    for (let i in proof) {
      root =
        ((index >>> i) & 0x1) == 1
          ? this.poseidon.F.toObject(this.poseidon([proof[i], root]))
          : this.poseidon.F.toObject(this.poseidon([root, proof[i]]))
    }
    return root;
  }
}
